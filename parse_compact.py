...
if coordinates:
    ...
    print(f"Bounding box: x=[{min_x}, {max_x_used}], y=[{min_y}, {max_y_used}]")
    print(f"Message dimensions: {max_x_used - min_x + 1} x {max_y_used - min_y + 1}\n")

    # Print just the bounding box region
    print("Secret message (full grid):")
    print("=" * 80)
    for y in range(min_y, max_y_used + 1):
        line = ''.join(grid[y][min_x:max_x_used + 1])
        print(line)
    print("=" * 80)

    # Also try to identify letters by looking at density
    print("\nLooking for concentrated regions (potential letters)...")
