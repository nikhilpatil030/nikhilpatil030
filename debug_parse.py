...
for i, match in enumerate(matches[:20]):
    ...
    char = match.group(2)
    y = int(match.group(3))
    print(f"{i+1}. x={x:3d}, char='{char}', y={y:3d}  (raw: '{match.group(0)}')")

print("\nX-coordinate range:", min(int(m.group(1)) for m in matches), "to", max(int(m.group(1)) for m in matches))
print("Y-coordinate range:", min(int(m.group(3)) for m in matches), "to", max(int(m.group(3)) for m in matches))

# Check unique X and Y values
x_values = sorted(set(int(m.group(1)) for m in matches))
y_values = sorted(set(int(m.group(3)) for m in matches))

print(f"\nUnique X values: {len(x_values)}")
print(f"Unique Y values: {len(y_values)}")
print(f"\nFirst 30 X values: {x_values[:30]}")
print(f"First 30 Y values: {y_values[:30]}")
